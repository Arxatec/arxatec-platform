...
test.describe("Verify Password Reset Page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(ROUTES.Auth.VerifyPasswordReset);
  });

  test("debe mostrar la página de verificación correctamente", async ({
    page,
  }) => {
    await expect(page).toHaveTitle(/Verificar cambio de contraseña - Arxatec/);
    await expect(
      page.getByText("Verificar cambio de contraseña")
    ).toBeVisible();
    await expect(
      page.getByText(
  ...
  test("debe redirigir al login después de verificación exitosa", async ({
    ...
    await page.route("**/auth/password-reset/verify-code", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ message: "Código verificado" }),
      });
    });
    ...
    await page.waitForURL(ROUTES.Auth.ChangePassword);
    await expect(page).toHaveURL(ROUTES.Auth.ChangePassword);
  });
